...
asociaciones <- read.delim("C:/Users/Elia/Desktop/genetica/TASSEL/analisis_ecococlimaticas/TB/LD_regiones/matriz LD TASSEL.txt", header=TRUE)
...
asociaciones_TFG <- read.csv("C:/Users/Elia/Desktop/genetica/TASSEL/analisis_ecococlimaticas/TB/LD_regiones/GWAS_Stats_fertility_bonferroni001.txt", sep="\t")
...
info_marker <- select(asociaciones_TFG, 1, 2, 3, 4, 6)
output_completo <- cbind(info_marker, output)

write.table(output_completo,file="intervalos_raw_Bonferroni.txt",sep="\t", row.names = FALSE)
