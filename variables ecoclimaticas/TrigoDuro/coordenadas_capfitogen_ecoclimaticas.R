...
Passaporte <- Passaporte[!(Passaporte$LATITU == "-"),] 
Passaporte[Passaporte$Specie == "dic",2] <- "dicoccon"
Passaporte[Passaporte$Specie == "d",2] <- "durum"
Passaporte[Passaporte$Specie == "t",2] <- "turgidum"
...
INSTCODE = substr(Passaporte$ID,1,3)
ACCENUMB = substr(Passaporte$ID,5,7)
...
SAMPSTAT <- c(rep(NA, nrow(Passaporte)))
ANCEST <- c(rep(NA, nrow(Passaporte)))
COLLSRC <- c(rep(NA, nrow(Passaporte)))
...
write_delim(df_pasaporte, "C:/Users/Elia/CAPFITOGEN3/Pasaporte/data.txt", delim = "\t", append = FALSE)
# lo generamos tambien en la carpeta donde estamos trabajando
write_delim(df_pasaporte, "data.txt", delim = "\t", append = FALSE)
...
bonferroni_tassel <- read.table("C:/Users/Elia/Desktop/genetica/TASSEL/analisis_ecococlimaticas/TD/GLM_Stats_MQ_Bonferroni_TD.txt", sep = ";", header = TRUE)
...
row.names(tabla_variables_extra)[nrow(tabla_variables_extra)] <- "GWAS"

## AÑADIR ASTERISCO EN LAS VARIABLES QUE HAN SALIDO SIGNIFICATIVAS EN ANOVA CON MARCADORES MOLECULARES DE LOS DATOS DE PATRI
anova_marcadores <- read.table("C:/Users/Elia/Desktop/genetica/analisis_patri/ANOVA_variables_ecoclimaticas/TD/anova_005_td.txt", sep = " ", header = TRUE)
...
row.names(tabla_variables_extra)[nrow(tabla_variables_extra)] <- "MARCADORES MOLECULARES"


write.table(tabla_variables_extra, "C:/Users/Elia/Desktop/genetica/coordenadas y variables ecoclimaticas/CAPFITOGEN3/analisis/TD/results/EcogeographicCharacterization_Spain/variables_ecogeograficas_extra.txt", sep = "\t", append = FALSE, row.names = TRUE)
...
unlink("pheno_ecoclimaticas_TD.txt")
#Seleccionar las 75 variables ecoclimaticas para los analisis y para lanzar el TASSEL
tabla_tassel <- tabla_variables %>% select(!c(DECLATITUDE, DECLONGITUDE, alt))
# tabla_tassel <- tabla_variables[,-2]
# tabla_tassel <- tabla_tassel[,-2]
